...
export default function MultipleImageUploader({ onChange }: { onChange: Dispatch<React.SetStateAction<never[] | (File | FileMetadata)[]>> }) {
  const maxSizeMB = 5;
  ...
  const [{ files, isDragging, errors }, { handleDragEnter, handleDragLeave, handleDragOver, handleDrop, openFileDialog, removeFile, getInputProps }] =
    useFileUpload({ accept: "image/svg+xml,image/png,image/jpeg,image/jpg,image/gif", maxSize, multiple: true, maxFiles });
