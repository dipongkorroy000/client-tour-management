...
import { Button } from "@/components/ui/button";
import { useCreateBookingMutation } from "@/redux/features/booling/booking.api";
...
const Booking = () => {
  ...
  const handleBooking = async () => {
    ...
    if (data?.[0]) {
      bookingData = await { tour: id, guestCount: guestCount };
      // --------
    }
    try {
  ...
  return (
    <>
      {!isLoading && isError && (
        <div>
          <p className="text-center text-red-500 my-10">Something Went Wrong!!</p>
        </div>
      )}

      {!isLoading && data?.length === 0 && (
        <div>
          <p className="text-center text-red-500 my-10">No Data Found</p>
        </div>
      )}
      ...
      {!isLoading && !isError && data!.length > 0 && (
        <div className="flex flex-col md:flex-row gap-8 p-6 container mx-auto">
          ...
          <div className="flex-1 space-y-6">
            <div>
              <img src={tourData?.images[0]} alt={tourData?.title} className="w-full h-64 object-cover rounded-lg" />
            </div>
          ...
          <div className="w-full md:w-96">
            <div className="border border-muted p-6 rounded-lg shadow-md sticky top-16">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex items-center space-x-3">
                    <button
                      onClick={() => setGuestCount((prv) => prv - 1)}
